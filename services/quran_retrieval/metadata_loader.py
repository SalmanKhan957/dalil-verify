...
from services.citation_resolver.surah_aliases import SURAH_CANONICAL_NAMES
...
def load_quran_metadata(csv_path: str | Path = DEFAULT_QURAN_ARABIC_PATH) -> dict[int, dict[str, Any]]:
    """Load lightweight Quran metadata from the canonical Arabic corpus.

    The loader derives ayah counts from the processed Arabic dataset so the
    citation resolver and retrieval layer use the same corpus-backed truth.
    """
    path = Path(csv_path)
    if not path.exists():
