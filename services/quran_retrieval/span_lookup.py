...
from services.quran_retrieval.metadata_loader import DEFAULT_QURAN_ARABIC_PATH


@lru_cache(maxsize=4)
def load_quran_row_index(csv_path: str | Path = DEFAULT_QURAN_ARABIC_PATH) -> dict[int, dict[int, dict[str, Any]]]:
    """Load Arabic Quran rows into a deterministic surah/ayah index."""
    path = Path(csv_path)
    if not path.exists():
...
def lookup_quran_span(
    *,
    ...
    """Return Arabic Quran rows for the requested ayah span in order."""
