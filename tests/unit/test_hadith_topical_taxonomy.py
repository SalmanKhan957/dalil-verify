...
def test_taxonomy_contains_seed_topics() -> None:
    assert 'rizq' in TOPIC_BY_SLUG
    assert 'anger' in TOPIC_BY_SLUG
    assert 'sleep' in TOPIC_BY_SLUG


def test_aliases_map_back_to_seed_topics() -> None:
    assert ALIAS_TO_TOPIC['rizk'] == 'rizq'
    assert ALIAS_TO_TOPIC['envy'] == 'hasad'
    assert ALIAS_TO_TOPIC['bedtime'] == 'sleep'
