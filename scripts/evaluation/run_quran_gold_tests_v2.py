...
def evaluate_one(test: dict[str, str], quran_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    retrieval_pass = actual_citation == expected_citation if expected_citation.strip() else True
    classification_pass = actual_status == expected_status
    overall_pass = mode_pass and retrieval_pass and classification_pass
...
def build_summary(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    retrieval_passed = sum(1 for r in rows if r["retrieval_pass"])
    classification_passed = sum(1 for r in rows if r["classification_pass"])
    overall_passed = sum(1 for r in rows if r["overall_pass"])
    ...
    return {
        ...
        "retrieval_pass_rate": round((retrieval_passed / total) * 100, 2) if total else 0.0,
        "classification_pass_rate": round((classification_passed / total) * 100, 2) if total else 0.0,
        "overall_pass_rate": round((overall_passed / total) * 100, 2) if total else 0.0,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Quran gold tests v2.")
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument(
        "--tests",
        type=str,
        default="data/gold_tests/quran_gold_tests_v2.csv",
    )
    parser.add_argument(
        "--out-dir",
        type=str,
    ...
    detailed_csv_path = Path(args.out_dir) / "quran_gold_test_results_v2.csv"
    summary_json_path = Path(args.out_dir) / "quran_gold_test_summary_v2.json"
